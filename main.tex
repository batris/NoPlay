...
\title{The No-Play Manifesto\footnote{\textbf{Please note that this is so far a very rough working document that only sketches parts of the discussion that will hopefully be exanded upon `real soon now'. Though it may be shown to interested parties, in its current state of development it should not be spread without the authors' explicit concent. }}}
...
Optimal video learning material is short and to the point. It can include interactive links. Automatically recorded two-hour lectures are a very poor substitute. Given that all of the above mentioned advantages, and some of the caveats, are better served by putting effort into producing good self study materials, including videos, staff should be encouraged to do this more than they are encouraged to allow the automatic systems to do make a poor job of it.
